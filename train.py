            ...
            if use_reg:
                ...
                for _ in range(config.training.group_size-1):
                    ...
                    for jj in range(5):
                        ...
                        x_batch_adv, trans_adv = attack.perturb(x_batch_reg, y_batch[0:num_grouped_ids], config.defense.reg_adv_exp_wrt, sess, jj)
                        y_batch_adv = y_batch[0:num_grouped_ids]
                        # construct new batches including rotated examples
                        # x_batch_inp = np.concatenate((x_batch, x_batch_adv), axis=0)
                        # y_batch_inp = np.concatenate((y_batch, y_batch_adv), axis=0)
                        # trans_inp = np.concatenate((noop_trans, trans_adv), axis=0)
                        # id_batch_inp = np.concatenate((id_batch, id_batch_adv), axis=0)
                        x_batch_inp = np.concatenate((x_batch_inp, x_batch_adv), axis=0)
                        y_batch_inp = np.concatenate((y_batch_inp, y_batch_adv), axis=0)
                        trans_inp = np.concatenate((trans_inp, trans_adv), axis=0)
                        id_batch_inp = np.concatenate((id_batch_inp, id_batch_adv), axis=0)
                    ...
                    if config.defense.cce_adv_exp_wrt != config.defense.reg_adv_exp_wrt:  # 3 fold batch
                        ...
                        # construct new batches including rotated examples
                        x_batch_inp = np.concatenate((x_batch_inp, x_batch_adv_cce), axis=0)
                ...
                if adversarial_training:
                    start = timer()
                    for jj in range(5):
                        # x_batch_inp, trans_inp = attack.perturb(x_batch, y_batch, "cce", sess, 0)
                        x_batch_adv, trans_adv = attack.perturb(x_batch, y_batch, "cce", sess, jj)
                        x_batch_inp = np.concatenate((x_batch_inp, x_batch_adv), axis=0)
                        y_batch_inp = np.concatenate((y_batch_inp, y_batch), axis=0)
                        trans_inp = np.concatenate((trans_inp, trans_adv), axis=0)
                        id_batch_inp = np.concatenate((id_batch_inp, id_batch), axis=0)
                ...
                # variables coincide
                # y_batch_inp = y_batch
                y_batch_adv = y_batch_inp
                trans_adv = trans_inp
                x_batch_adv = x_batch_inp
                # id_batch_inp = id_batch
                id_batch_adv = id_batch_inp
